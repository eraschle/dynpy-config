...
class PathEntryFunctions(tk.Frame):
    ...
    def setup(self, args: utils.UiArgs):
        utils.setup_button(
            button=self.btn_add, text="󰐖", command=self.controller.path_entry_add_command
        )
        self.btn_add.grid(cnf=self.args.as_grid())
        self.args.add_row()
        utils.setup_button(
            button=self.btn_remove, text="󰍵", command=self.controller.path_entry_remove_command
        )
        self.btn_remove.grid(cnf=self.args.as_grid())
        self.args.add_row()
        utils.setup_button(
            button=self.btn_save, text="", command=self.controller.path_entry_save_command
        )
        self.btn_save.grid(cnf=self.args.as_grid())
        self.args.add_row()
        utils.setup_button(
            button=self.btn_open, text="󰙅", command=self.controller.path_entry_open_command
        )
