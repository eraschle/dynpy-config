...
def path_to_wsl(path: str) -> str:
    if path.startswith(PATH_WSL):
        return path
    path = path.removeprefix(PATH_WIN)
    return f"{PATH_WSL}{path}"


def path_to_win(path: str) -> str:
    if path.startswith(PATH_WIN):
        return path
    path = path.removeprefix(PATH_WSL)
    return f"{PATH_WIN}{path}"
...
class PythonConfigController:
    ...
    def _get_selected_id(self, event: tk.Event) -> str:
        ...
        if tree is None:
            raise Exception(f"event widget should be Treeview but is {type(event.widget)}")
    ...
    def _current_parent(self, current: PathEntryViewModel) -> PathFileViewModel:
        return cast(PathFileViewModel, self.uuid[current.parent_uuid])
    ...
    def show_in_explorer(self, path: str):
        path = path_to_wsl(path)
        if not os.path.exists(path):
            return
        win_dir = str(os.getenv('WINDIR', ""))
        explorer = os.path.join(win_dir, 'explorer.exe')
        path = path_to_win(path)
        path = os.path.normpath(path)
        subprocess.run([explorer, '/select,', path], check=False)

    def _create_python_path(self, model: PathFileViewModel) -> PythonPathFile:
    ...
    def _create_python_path_files(self) -> List[PythonPathFile]:
        models = [self._create_python_path(model) for model in self._path_file_models()]
        return models
